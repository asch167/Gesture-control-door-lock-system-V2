...
def index():
    ...
    if request.method =='POST':
        if request.values['send']=='送出':
            ...
            #----------------------------
            return render_template('login-page.html',tip=f"密碼已設定完成為:{Password} 有效時間為{Survival_Time}秒")
...
def main():
    ...
    while True:
        ...
        if Password: # 若設定的密碼存在
            if str(FingerNum.dtnum) != "Great": # 會加上and是因為要是沒比動作會直接回傳None所以這樣寫能防止輸入None
                Entered += str(FingerNum.dtnum) if FingerNum.dtnum else ""
                oled_control(['Enter Password:',Entered])
                # print("已輸入密碼 :", Entered)
            elif str(FingerNum.dtnum) == "Great" and FingerNum.dtnum:
                if Password == Entered:
                    Entered = ""  #  輸入的密碼清除
